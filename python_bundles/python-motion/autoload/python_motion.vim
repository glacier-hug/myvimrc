fun! python_motion#move(pattern, flags, ...) "{{{
    let cnt = v:count1 - 1
...
fun! python_motion#vmove(pattern, flags) range "{{{
    call cursor(a:lastline, 0)
    let end = python_motion#move(a:pattern, a:flags)
...
fun! python_motion#pos_le(pos1, pos2) "{{{
    return ((a:pos1[0] < a:pos2[0]) || (a:pos1[0] == a:pos2[0] && a:pos1[1] <= a:pos2[1]))
endfunction "}}}


fun! python_motion#select(pattern, inner) "{{{
    let cnt = v:count1 - 1
    let orig = getpos('.')[1:2]
    ...
    while cnt
        ...
        let cnt = cnt - 1
    endwhile
    if python_motion#pos_le([snum, 0], orig) && python_motion#pos_le(orig, [enum, 1])
        if a:inner
            let snum = snum + 1
